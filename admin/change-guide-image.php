...
if (strlen($_SESSION['alogin']) == 0) {
    ...
    $imgid = intval($_GET['imgid']);  // Guide's NationalID or unique identifier

    if (isset($_POST['submit'])) {
        $gimage = $_FILES["guideimage"]["name"];
        move_uploaded_file($_FILES["guideimage"]["tmp_name"], "guideimages/" . $_FILES["guideimage"]["name"]);

        $sql = "UPDATE tblTourGuides SET Image=:gimage WHERE NationalId=:imgid";
...
?>
    ...
    <head>
        <title>TravelEase | Admin Guide Image Update</title>
                        ...
                        <div class="tab-content">
                            <div class="tab-pane active" id="horizontal-form">
                                <form class="form-horizontal" name="guide" method="post" enctype="multipart/form-data">
                                    ...
                                    $sql = "SELECT Image from tblTourGuides WHERE NationalId=:imgid";
                                    $query = $dbh->prepare($sql);
                                    $query->bindParam(':imgid', $imgid, PDO::PARAM_STR);
                                    $query->execute();
                                    ...
                                    if ($query->rowCount() > 0) {
                                        foreach ($results as $result) { ?>
                                            <div class="form-group">
                                                ...
                                                <div class="col-sm-8">
                                                    <img src="guideimages/<?php echo htmlentities($result->Image); ?>" width="200">
                                                </div>
                                            </div>
